...
for i in trange(2000):
    r = randint(0, len(data['data'])-1)
    x_train.append(data['data'][r])
    y_train.append(data['label'][r])
for i in trange(1000):
    r = randint(0, len(data['data'])-1)
    x_test.append(data['data'][r])
    y_test.append(data['label'][r])
...
try:
    ...
    model.add(Conv2D(5, kernel_size=(3, 3), strides=(1, 1),
                     ...
                     # input_shape=(28, 28, 1),
                     name='C1'))
    model.add(MaxPooling2D(pool_size=(2, 2), strides=(1, 1), name='S1'))
    model.add(Conv2D(7, (3, 3), activation='relu', name='C2'))
    model.add(MaxPooling2D(pool_size=(2, 2), name='S2'))
    model.add(Conv2D(9, (3, 3), activation='relu', name='C3'))
